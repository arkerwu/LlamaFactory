...
分析脚本会标记最大 loss 超过 `mean + 2*std`（可通过 `--z-threshold` 调整）且不低于所属数据集损失中位数的样本——低于本数据集中位数说明是该数据集整体偏高，不算单点异常（多 epoch 训练时同一样本的多次记录会先按样本聚合），并输出各数据集的损失概览（记录数 / 样本数 / 均值 / 中位数 / 异常数，损失按样本等权统计，可对照发现整个数据集整体偏高的情形）以及 top-N 异常清单（`--top`，每行为样本损失及其所属数据集的损失中位数，便于判断单点异常幅度），用于数据清洗。若训练前期 loss 波动较大，可用 `--min-step <N>` 忽略 `global_step` 小于 N 的记录，只看训练后期相对稳定的阶段。
